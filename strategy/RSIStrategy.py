...
class RSIStrategy(QThread):
    ...
    def init_strategy(self):
        """전략 초기화 기능을 수행하는 함수"""
        try:
            # 유니버스 조회, 없으면 생성 (DB 생성, self.universe 생성)
    ...
    # (DB table) universe 존재 검사 => 없으면 크롤링 결과로 (table)universe 생성
    def check_and_get_universe(self):
        """DB에 유니버스가 존재하는지 확인하고 없으면 DB File 생성하는 함수"""
        if not check_table_exist(self.strategy_name, 'universe'):

            # universe_list : 종목 이름만 가져옴 : universe는 사용자가 지정한 조건에 맞는 종목만 발굴한 것
            universe_list = get_universe()
            print("유니버스 목록 가져오기 > universe_list : ",  universe_list)
            universe = {}

            # 오늘 날짜를 20210101 형태로 지정
            now = datetime.now().strftime("%Y%m%d")

            # KOSPI(0)/KOSDAQ(10)에 상장된 모든 종목 코드를 가져와 kospi_code_list에 저장
            kospi_code_list = self.kiwoom.get_code_list_by_market("0")
            kosdaq_code_list = self.kiwoom.get_code_list_by_market("10")

            print("from API get 종목코드 where 크롤링")
            for code in kospi_code_list + kosdaq_code_list:                                         # 1. 종목코드 추출 (from API) 코스피/코스탁 등록된 모든 종목 코드
                # 모든 종목 코드를 바탕으로 반복문 수행
                code_name = self.kiwoom.get_master_code_name(code)                                  # 2. 종목명 추출 (from API)
        ...
        universe_list = cur.fetchall()                                                              # 6. 레코드셋 생성(universe_list)
        for item in universe_list:
            idx, code, code_name, created_at = item
            self.universe[code] = {                                                                 # 7. 지역변수(universe) 내용을 전역변수(self.universe)에 저장 >> {'019180',{'code_name','티에이치엔'}}
                'code_name': code_name
            }
        print("self.universe : " , self.universe)
